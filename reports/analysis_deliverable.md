...
Despite the poor performance of our KNN model, we believe that our results confirm our initial hypothesis that there is a significant and predictive relationship between academic performance, health, and food desert status. While our KNN performed rather poorly, we were able to learn some signal as seen by our balanced accuracy converging around 40% from the baseline 33%. Additionally, our supervised learning model (XGBoost) was able to achieve achieve >80% accuracy for both the math and reading/language arts subsets of our data with supporting precision, recall, and f1 scores. This suggests that there is a strong predictive relationship in our data. One aspect of our data which could have lead to the poor performance of our unsupervised model is the lack of data diversity. The academic performance data contains many features which are all somewhat related and we only had two features from the health dataset. Adding more features regarding quality of life, school metrics, and other related data may have provided a more robust representation of the underlying task for our KNN to train on. Additionally, our target variable had a significant imbalance which, even with out weighted training scheme, could have led to issues in generalization beyond our training data. One way we could have addressed this could have been by engineering our target variable by calculating our own thresholds for urban and food desert classifications instead of relying on our data's raw classification. Finally, we believe that our models and training methods were appropriate for the task. Our supervised model with our weighted training scheme was able to learn a strong signal from our data and our unsupervised method yielded contrasting but informative results about our data.
